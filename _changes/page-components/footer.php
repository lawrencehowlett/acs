<footer class="footer">
	<div class="inside cols teasers">
		<article class="col col3 footer-teaser" style="background-image: url('img/temp/photos/teaser01.jpg');">
			<h3 class="footer-teaser-title">Speak to an ACS specialist</h3>
			<p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. </p>
			<p class="more"><a href="">Live chat</a></p>
		</article>
		<article class="col col3 footer-teaser" style="background-image: url('img/temp/photos/teaser02.jpg');">
			<h3 class="footer-teaser-title">ROI calculator</h3>
			<p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. </p>
			<p class="more"><a href="blog.php">ACS Blog</a></p>
		</article>
		<article class="col col3 footer-teaser" style="background-image: url('img/temp/photos/teaser03.jpg');">
			<h3 class="footer-teaser-title">Contact us</h3>
			<p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. </p>
			<p class="more"><a href="contact.php">Contact us</a></p>
		</article>
	</div>
	<div class="inside cf">
		<nav class="footer-section cols footer-nav">
			<div class="col footer-menu">
				<h4 class="footer-menu-title">Services</h4>
				<ul class="menu">
					<li><a href="it-solutions.php">IT Solutions</a></li>
					<li><a href="#">Cloud 365</a></li>
					<li><a href="workplace.php">Workplace</a></li>
					<li><a href="#">Recruitment</a></li>
				</ul>
			</div>
			<div class="col footer-menu">
				<h4 class="footer-menu-title">Customers</h4>
				<ul class="menu">
					<li><a href="#">Sign in</a></li>
					<li><a href="#">Support</a></li>
					<li><a href="#">FAQs</a></li>
					<li><a href="#">Testimonials</a></li>
				</ul>
			</div>
			<div class="col footer-menu">
				<h4 class="footer-menu-title">About us</h4>
				<ul class="menu">
					<li><a href="#">Our Story</a></li>
					<li><a href="#">Offices</a></li>
					<li><a href="#">Press</a></li>
				</ul>
			</div>
		</nav>
		<section class="footer-section">
			<p class="cta">
				<a href="speak-to-specialist.php" class="cta-btn">Speak to specialist</a>
			</p>
		</section>
		<section class="footer-section social-section cf">
			<ul class="social">
				<li><a href="#" class="google-plus"><div class="img"><img src="img/temp/social-googleplus.png" alt=""></div>Google+</a></li>
				<li><a href="#" class="youtube"><div class="img"><img src="img/temp/social-youtube.png" alt=""></div>YouTube</a></li>
				<li><a href="#" class="twitter"><div class="img"><img src="img/temp/social-twitter.png" alt=""></div>Twitter</a></li>
				<li><a href="#" class="linkedin"><div class="img"><img src="img/temp/social-linkedin.png" alt=""></div>LinkedIn</a></li>
				<li><a href="#" class="vine"><div class="img"><img src="img/temp/social-vine.png" alt=""></div>Flickr</a></li>
			</ul>
			<ul class="menu legal">
				<li><a href="#">Privacy</a></li>
				<li><a href="#">Terms</a></li>
			</ul>
			<div class="copy">
				<p class="copyright">Copyrights &copy; 2015 ACS Systems (UK) Ltd</p>
				<p class="copyright">Responsive Web Design by <a href="https://www.newedge.co.uk/">Newedge</a></p>				
			</div>
		</section>
	</div>
</footer>
<script type='text/javascript' src='https://maps.googleapis.com/maps/api/js?v=3.exp&amp;sensor=false'></script>
<!-- <script type="text/javascript" src="js/lib/jquery.js"></script>
<script type="text/javascript" src-"js/lib/underscore.js"></script>
<script type="text/javascript" src="js/lib/hammer.js"></script>
<script type="text/javascript" src="js/lib/hammer.jquery-plugin.js"></script>
<script type="text/javascript" src="js/scripts/navigated-slider.js"></script>
<script type="text/javascript" src="js/lib/jquery.bxslider.min.js"></script>
<script type="text/javascript" src="js/lib/jquery-custom-content-scroller.js"></script>
<script type="text/javascript" src="js/scripts/double-slider.js"></script>
<script type="text/javascript" src="js/scripts/gallery-slider.js"></script>
<script type="text/javascript" src="js/scripts/utils.js"></script>
<script type="text/javascript" src="js/scripts/history.js"></script>
<script type="text/javascript" src="js/scripts/contact.js"></script>
<script type="text/javascript" src="js/scripts/fluid-videos.js"></script>
<script type="text/javascript" src="js/scripts/youtube.js"></script> -->
<script type="text/javascript" src="js/min.js"></script>
<!-- <script type="text/javascript" src="js/min.js"></script> 
<script type='text/javascript' src='js/lib/marker-with-label-packed.js'></script>-->
</body>
</html>